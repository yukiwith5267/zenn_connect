...
- Unityで実装例を見ることで、強化学習サイクルを理解する。
- 学習の効率化を図るために、学習設定ファイルで設定する「ハイパーパラメータ」の役割と調整方法を理解する。
...
| 観察 | ドローンのRigidbodyの速度ベクトル / 現在の回転角度（オイラー角) |
...
今回は「ml-agents-release_20」フォルダの「config」フォルダ直下に、「drone_training.yaml」ファイルを作成し、以下を記述します。
