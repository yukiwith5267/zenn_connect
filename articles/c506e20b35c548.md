...
ML-Agentsとは[Unity Machine Learning Agents](https://unity.com/ja/products/machine-learning-agents)のことで、
Unity を使って機械学習を行うことのできるフレームワークです。

https://github.com/Unity-Technologies/ml-agents
から「Release 20」の「download」からダウンロードし、Unityのパッケージマネージャーからインポートします。
...
```
#### 衝突検出
Playerがゲーム内のトリガーコライダーと衝突するたびに、`OnTriggerEnter`メソッドが呼び出されます。
壁のタグを`Wall`、チェックポイントのタグを`CheckPoint`に設定し、
タグが`CheckPoint`のオブジェクトに衝突した場合、報酬を追加し、`cptCount`を増やします。すべてのチェックポイントを通過した場合、ゴール報酬を追加し、エピソードを終了します。
